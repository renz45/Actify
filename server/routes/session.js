...
module.exports = function(app) {
  app.get('/sign_in', function(req, res) {
    res.sendfile('session/sign_in.html', {root: app.settings.views});
  });

  app.get('/sign_up', function(req, res) {
    res.sendfile('session/sign_up.html', {root: app.settings.views});
  });
  ...
  app.post('/registration', function(req, res) {
    ...
    if(password === req.param('password_confirm')) {
      ...
      // User.findOrCreate({type: 'user', name: req.param('name'), username: req.param('username'), password: encryptedPassword}).success(function(user) {
